...
const PhoneMyCourses = () => {
  ...
  const fetchUserCourses = async (uid) => {
    try {
      ...
      const coursePromises = courseIds.map(async (courseId) => {
          ...
          chapters.map(async (chapter, chapterIndex) => {
            const topics = await Promise.all(
              chapter.topics.map(async (topic, topicIndex) => ({
                ...
                videoLinks: (topic.videoLinks || []).map(
                  (video, videoIndex) => ({
                    ...video,
                    id: `video-${chapterIndex + 1}-${topicIndex + 1}-${
                      videoIndex + 1
                    }`,
                  })
                ),
        ...
        const videoProgressSnapshot = await getDocs(
          collection(db, "users", uid, "courses", courseId, "videoProgress")
        );
        const videoProgress = {};
      ...
      const courseList = (await Promise.all(coursePromises)).filter(
        (course) => course !== null
      );
  ...
  const fetchCourseData = async (courseId) => {
    try {
      ...
      const videoProgressSnapshot = await getDocs(
        collection(db, "users", userUid, "courses", courseId, "videoProgress")
      );
      const videoProgress = {};
      videoProgressSnapshot.forEach((videoDoc) => {
        const data = videoDoc.data();
  ...
  const fetchVideoProgress = async (courseId) => {
    try {
      const videoProgressCollection = collection(
        db,
        "users",
        userUid,
        "courses",
      ...
      const lastWatchedDoc = await getDoc(
        doc(
          collection(
            db,
            "users",
      ...
      if (lastWatchedDoc.exists()) {
        ...
        if (lastWatchedVideo) {
          ...
          setSelectedVideoDescription(
            lastWatchedVideo.description || "No description available"
          );
        ...
        for (const video of topic.videoLinks || []) {
          if (video.id === videoId) {
            setExpandedChapters((prev) => ({
              ...prev,
              [chapter.chapterName]: true,
            }));
            setExpandedTopics((prev) => ({
              ...prev,
              [`${chapter.chapterName}-${topic.topicName}`]: true,
            }));
          }
        }
      }
  ...
  const toggleTopic = (chapterName, topicName) => {
    setExpandedTopics((prevState) => ({
      ...
      [`${chapterName}-${topicName}`]:
        !prevState[`${chapterName}-${topicName}`],
    }));
  };
  ...
  const handleVideoEnded = () => {
    ...
    if (nextVideo) {
      ...
      setSelectedVideoDescription(
        nextVideo.description || "No description available"
      );
      ...
      for (const topic of chapter.topics) {
        for (const item of topic.videoLinks) {
          if (item.id === video.id) {
            return (
              previousVideo === null ||
              completedVideos.includes(previousVideo.id)
            );
          }
          previousVideo = item;
        }
  ...
  const handleFullScreenChange = () => {
    if (document.fullscreenElement) {
      // Rotate video to landscape mode
      if (videoRef.current) {
        videoRef.current.style.transform = "rotate(90deg)";
        videoRef.current.style.transformOrigin = "center";
        videoRef.current.style.width = "100vh";
        videoRef.current.style.height = "100vw";
      }
    } else {
      // Reset video style when exiting full-screen
      if (videoRef.current) {
                  ...
                  {chapter.topics.map((topic) => (
                    <div key={topic.id} className="mb-4">
                      <div
                        ...
                        } rounded-lg cursor-pointer hover:bg-blue-500 hover:text-white transition-colors duration-300`}
                        onClick={() =>
                          toggleTopic(chapter.chapterName, topic.topicName)
                        }
                      >
